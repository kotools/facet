...
- The `@GeneratedKotoolsFacetApi` annotation to `facet-core-dsl`, indicating
  that a declaration is only intended for code generated by Kotools Facet.
- The `FacetPropertyScope` class to `facet-core-dsl`, being the receiver of the
  block passed to a facet's property operations.
- The `BidirectionalFacet` interface to `facet-core-dsl`, marking a nested
  declaration as generating a bidirectional projection for its containing
  _faceted class_, and its associated code generation to the `facet-core-ksp`
  module.
